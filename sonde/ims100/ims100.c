...
ims100_decode(IMS100Decoder *self, SondeData *dst, const float *src, size_t len)
{
	...
	case READ:
		...
		if (IMS100_DATA_VALID(self->frame.valid, IMS100_MASK_SEQ)) {
			dst->fields |= DATA_SEQ;
			dst->seq = ims100_frame_seq(&self->frame);
		}

		/* Parse PTU data {{{ */
		if (IMS100_DATA_VALID(self->frame.valid, IMS100_MASK_PTU)) {
			...
			dst->pressure = 0;
			dst->calibrated = IMS100_DATA_VALID(self->calib_bitmask, IMS100_CALIB_PTU_MASK);
