...
{
	struct nk_color prev_text_color;
	float az, el, range;
	char tmp[128];

	const SondeData *printable = get_data();
	...
	strftime(tmp, sizeof(tmp), "%a %b %d %Y %H:%M:%S", gmtime(&printable->time));
	nk_label(ctx, tmp, NK_TEXT_LEFT);
	...
	if ((lat || lon || alt) && (printable->fields & DATA_POS))  {
	...
	/* If not fully calibrated, grey out the PTU data */
	if (printable->calib_percent < 100.0) {
		prev_text_color = ctx->style.text.color;
		ctx->style.text.color.r = 127;
		ctx->style.text.color.g = 127;
		ctx->style.text.color.b = 127;
	}
	...
	if (printable->fields & DATA_XDATA) {
		nk_label(ctx, "Aux. data: ", NK_TEXT_RIGHT);
		sprintf(tmp, "O3=%.2f ppb", printable->xdata.o3_ppb);
		nk_label(ctx, tmp, NK_TEXT_LEFT);
	}

	/* If not fully calibrated, grey out the PTU data */
	if (printable->calib_percent < 100.0) {
		ctx->style.text.color = prev_text_color;
	}
